...
보안 때문에 SDK 사용을 위해서는 인증이 필요하다. 위 API를 호출하면 유저의 토큰(i.e. 비밀번호)을 생성하여 퍼블리셔 서버로 전달한다. 퍼블리셔 서버는 해당 토큰을 받아서 퍼블리셔 DB에 저장해야 한다. 이 저장된 유저 토큰을 SDK 사용 시 함수 파라미터로 전달해야 한다. 이 API 사용을 위해서는 먼저 퍼블리셔 서버의 아이피 주소를 `화이트 리스트`로 등록해야 한다. 화이트 리스트에 등록 될 아이피주소는 별도의 채널(e.g. 이메일)을 통해서 퍼블리셔가 전달한다.
